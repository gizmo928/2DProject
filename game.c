...
int main(int argc, char * argv[])
{
	...
	grounds = gf2d_sprite_load_all("images/ground.png", 100, 100, 1);
	...
	bg = Mix_LoadMUS("sounds/root.wav");
	...
	break_bonesFX = Mix_LoadWAV("sounds/bb.wav"); //isnt working
	...
	Mix_VolumeChunk(gruntFX, 10);

	ground = Ent_new();
	ground->sprite = grounds;
	ground->position = vector2d(400, 420);
	ground->scale = vector2d(10, 1);
	...
	//temp2->inuse = 1;
	temp2->sprite = cavem;
	temp2->position = vector2d(10, 100);
	...
	while (!done)
	{
		...
		{	
			...
			if (ev.button.button == SDL_BUTTON_LEFT && pressed >= 40 && weapon_choice == 1)
			{
				...
				//Vector3D * hi = & W_spear[count]->rotation;
				//vector3d_set_angle_by_radians(hi, angle[count]);
				//dx = mx - W_spear[count]->position.x; // finds the difference of the mouse and spear sprite on x axis
				//dy = my - W_spear[count]->position.y;
				...
				/*
				 ...
				 if (mx != 0)
				 {
					 angle = atan(dy/dx); // could be int or w/e to go inside vector3d
				 }
				 else if (my - W_spear[count]->position.y < 0)
				 {
					 angle = 90;
				 }
				 else
				...
				W_spear[count]->rotation = vector3d(0, 0, 100); 
				W_spear[count]->position = vector2d(dx, dy);
			...
			else if (ev.button.button == SDL_BUTTON_LEFT && pressed >= 40 && weapon_choice == 3) 
			{
				club->position = vector2d(temp2->position.x + 20, temp2->position.y -15 );
		...
		*/
		...
		if (ifCollision(W_spear[0], temp) == false && ifCollision(W_spear[1], temp) == false && ifCollision(W_spear[2], temp) == false && ifCollision(W_spear[3], temp) == false && ifCollision(W_spear[4], temp) == false
	)
			{
				temp->color = vector4d(255, 255, 255, 255);
			}
			...
			SPEAR DAMAGES TREX
			*/
			if (ifCollision(W_spear[0], temp4) == false && ifCollision(W_spear[1], temp4) == false && ifCollision(W_spear[2], temp4) == false && ifCollision(W_spear[3], temp4) == false && ifCollision(W_spear[4], temp4) == false  ) // if spear hits the fino
			{
				temp4->color = vector4d(255, 255, 255, 255);
			}
			else {
			...
			SPEAR DAMAGES PTERO
			*/
			if (ifCollision(W_spear[0], ent_ptero) == false && ifCollision(W_spear[1], ent_ptero) == false && ifCollision(W_spear[2], ent_ptero) == false && ifCollision(W_spear[3], ent_ptero) == false && ifCollision(W_spear[4], ent_ptero) == false) // if spear hits the fino
			{
				ent_ptero->color = vector4d(255, 255, 255, 255);
			}
			else {
		...
		/* ROCK DAMAGES DINO*/
			...
			if (ifCollision(rock[0], temp) == false && ifCollision(rock[1], temp) == false && ifCollision(rock[2], temp) == false && ifCollision(rock[3], temp) == false && ifCollision(rock[4], temp) == false
				) 
			{
				temp->color = vector4d(255, 255, 255, 255);
			}
			...
			ROCK DAMAGES TREX
			*/
			if (ifCollision(rock[0], temp4) == false && ifCollision(rock[1], temp4) == false && ifCollision(rock[2], temp4) == false && ifCollision(rock[3], temp4) == false && ifCollision(rock[4], temp4) == false) // if spear hits the fino
			{
				temp4->color = vector4d(255, 255, 255, 255);
			}
			else {
			...
			ROCK DAMAGES PTERO
			*/
			if (ifCollision(rock[0], ent_ptero) == false && ifCollision(rock[1], ent_ptero) == false && ifCollision(rock[2], ent_ptero) == false && ifCollision(rock[3], ent_ptero) == false && ifCollision(rock[4], ent_ptero) == false) // if spear hits the fino
			{
				ent_ptero->color = vector4d(255, 255, 255, 255);
			}
			else {
